...
export function MobileBottomNav({ activeTab, onNavigate }: MobileBottomNavProps) {
  ...
  return (
    <View className="bg-[#f5f8ff] px-3 pt-2" style={{ backgroundColor: tokens.background, paddingBottom: Math.max(insets.bottom, 10) }}>
      <View className="flex-row items-stretch gap-1 rounded-[24px] border border-[#dfe8f4] bg-white px-2 py-2 shadow-sm" style={{ borderColor: tokens.border, backgroundColor: tokens.surface }}>
        {items.map((item) => {
          ...
          return (
            <Pressable
              ...
              style={{ backgroundColor: isActive ? tokens.primarySoft : 'transparent' }}
            >
              <View
                className={`h-9 w-9 items-center justify-center rounded-2xl ${
                  isActive ? 'bg-[#2f64f6]' : 'bg-[#f2f6fd]'
                }`}
                style={{ backgroundColor: isActive ? tokens.primary : tokens.surfaceElevated }}
              >
                <FontAwesomeIcon
                  icon={item.icon}
                  size={15}
